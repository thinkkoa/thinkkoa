...
const checkEnv = function () {
    ...
    if (lib.toNumber(node_engines) > lib.toNumber(nodeVersion)) {
        console.error(`ThinkKoa need node version > ${node_engines}, current version is ${nodeVersion}, please upgrade it.`);
        process.exit(-1);
    }
};
...
class ThinkKoa extends koa {
    ...
    init(options) {
        ...
        process.env.APP_DEBUG = this.app_debug;
    ...
    config(name, type = 'config') {
        try {
            let caches = this._caches.configs || {};
