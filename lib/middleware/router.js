 ...
 * @returns 
 */
const getPathname = function getPathname(ctx, options) {
 ...
 * @returns 
 */
const cleanPathname = function cleanPathname(pathname) {
    if (pathname === '/') {
        return '';
    }
    if (pathname[0] === '/') {
 ...
 * @returns 
 */
const parsePathname = function parsePathname(ctx, pathname, options, multi = false) {
    if (pathname) {
        pathname = cleanPathname(pathname);
        let paths = pathname.split('/') || [],
            modules = think._caches.modules || [],
            group = '';
        if (multi) {
 ...
 * @returns 
 */
const parseGroup = function parseGroup(group, value = 'home') {
    if (!group) {
        return value || 'home';
    }
 ...
 * @returns 
 */
const parseController = function parseController(controller, value = 'index') {
    if (!controller) {
        return value || 'index';
    }
 ...
 * @returns 
 */
const parseAction = function parseAction(action, value = 'index') {
    if (!action) {
        return value || 'index';
    }
 ...
 * @param {any} options 
 */
const parseRoute = function parseRoute(ctx, routers, options) {
    let keys,
        regexp,
        regres,
        index,
