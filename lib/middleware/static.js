...
const safeDecodeURIComponent = function safeDecodeURIComponent(text) {
    try {
...
const loadFile = function loadFile(name, dir, option, files) {
...
module.exports = function (options) {
    ...
    // custom files list
    let files = options.files || (0, _create2.default)(null);
    let filePrefix = path.normalize(options.prefix.replace(/^\//, ''));

    // option.filter
    let fileFilter = function fileFilter() {
        return true;
    };
    ...
    if (Array.isArray(options.filter)) {
        fileFilter = function fileFilter(file) {
    ...
    // alias files
    if (options.alias) {
        (0, _keys2.default)(options.alias).forEach(function (key) {
